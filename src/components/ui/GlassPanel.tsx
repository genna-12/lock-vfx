 ...
 * niente. Lì la card **tiene il fondo `obsidian` e il bordo** e perde solo il
 * cenno e la luce (Direzione, 16/9 — §9.4): il volume è la card, non il
 * movimento, e sul telefono non deve diventare un rettangolo vuoto. Nessun
 * listener, nessun nodo in più.
 *
...
export function GlassPanel({ children, className = '' }: GlassPanelProps) {
  ...
  if (!vivo) return <section className={`bg-obsidian ${className}`}>{children}</section>;
